...
bool Tilemap::load(std::string filename)
{
	...
	if (!_file) {
		...
		std::vector< Block_Portal > p_start;
		std::vector< Block_Portal > p_end;
		...
		for (size_t y = 0; std::getline(_file, line); y++) {
			...
			for (size_t x = 0; std::getline(ss_line, chars, ','); x++) {
				...
				if (chars[0] == 'P') {
					...
					Block_Portal curr_portal;
					curr_portal.setSize((float)TILE_SIZE);
					curr_portal.setPosition((float)x*TILE_SIZE, (float)y*TILE_SIZE);
		...
		for (size_t i = 0; i < p_start.size(); i++) {
			Block_Portal block_start = p_start[i];
			block_start.createPortal(p_end[i].getPosition());

			Block_Portal block_end = p_end[i];
			block_end.createPortal(p_start[i].getPosition());

			_bounds.push_back(block_start);
			_bounds.push_back(block_end);
		}
...
std::pair<Tilemap::Collision_Type, sf::Vector2f> Tilemap::collision(sf::FloatRect player, bool can_teleport)
{
	...
	for (size_t i = 0; i < _bounds.size(); i++) {
		...
		// If there is a collision with a portal Block
		// Returns the collision type Portal and the destination point of the portal
		if (player.intersects(_bounds[i].getGlobalBounds()) && _bounds[i].isPortal() && can_teleport) {
			return std::make_pair(Collision_Type::portal, _bounds[i].getTeleportPoint());
		}
